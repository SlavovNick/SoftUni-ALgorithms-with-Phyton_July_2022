...
class Edge:
    ...
    def __gt__(self, other): # The Priority queue will use this function to order the elements
...
def prim(node, damage, damage_by_node, graph):
    ...
    [pq.put(edge) for edge in graph[node]] # Put in The Priority Queue all the edges, that starting from begining node

    while not pq.empty():
        min_edge = pq.get()#using GT function
...
graph = {node: [] for node in range(nodes)} # Initialisation of graph with all nodes and empty lists for every node
#reading of all edges
