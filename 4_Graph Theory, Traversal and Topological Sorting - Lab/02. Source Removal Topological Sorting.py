...
def find_node_without_dependencies(dependancies_by_node):
    for node, dependencies in dependancies_by_node.items():
        if dependencies == 0:
            return node
    return None
...
while dependancies_by_node:
    node_to_remove = find_node_without_dependencies(dependancies_by_node)
