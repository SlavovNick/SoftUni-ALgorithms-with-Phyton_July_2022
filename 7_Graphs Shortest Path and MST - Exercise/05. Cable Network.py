...
for _ in range(edges):
    ...
    if len(edge_data) == 4: #adding connected nodes to tree
        tree.add(first)
        tree.add(second)

pq = PriorityQueue()

for node in tree: #adding all connected edges to Priority Queue
...
while not pq.empty():
    ...
    if budget_used + min_edge.weight > budget: #we can't use this edge (budget)
        break

    budget_used += min_edge.weight # we can use this edge (budget)
